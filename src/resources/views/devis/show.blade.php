    ...
    <section class="mx-auto flex h-full w-[100%] flex-col items-center justify-between bg-white py-10">
        <div class="flex w-full flex-col items-center justify-start gap-4">
                ...
                <div class="flex flex-col items-end justify-between gap-8 border-l-4 border-gray-300">
                    ...
                    {{-- <div
                        ...
                        <div class="text-2xl font-bold w-64 flex items-end justify-between">
                            <p class="text-4xl">Devis N°:<span class="font-normal"> {{ $devis->id }}</span></p>
                        </div>
                        <div class="text-xl font-bold w-32 flex items-end justify-between gap-2">
                            <p class="">Date:</p>
                            <p class="font-normal">{{ $devis->created_at }}</p>
                        </div>
                    </div>
                    <div class="text-end text-sm">
                        <p class="font-bold">{{ $devis->user->name }}</p>
                        <p>{{ $devis->user->ville }}</p>
            ...
            <div class="w-full px-12 grid grid-cols-3 gap-6">
                <h1 class="col-span-3 text-2xl font-black text-center">Devis</h1>
                @php
                    $offset = 5 - ($devis->id !== 0 ? floor(log10($devis->id)) : 1);
                    $id = $offset >= 0 ? str_pad($devis->id, $offset, '0', STR_PAD_LEFT) : $devis->id;
                @endphp
                <p class="text-start"><span class="font-bold text-neutral-700">Devis N °: </span>{{ $id }}
                </p>
                <p class="text-center"><span class="font-bold text-neutral-700">Référence: </span>{{ 'EST-' . $id }}
                </p>
                <p class="text-end"><span class="font-bold text-neutral-700">Date: </span>{{ $devis->created_at }}
                </p>
            </div>
            ...
            <div class="w-full px-12 gap-6">
                ...
                <div class="mt-4 grid grid-cols-2 gap-4 text-start text-md">
                    <p class="font-bold text-gray-700">{{ $devis->user->name }}</p>
                    <p><span class="font-bold text-gray-700">ICE: </span>{{ $devis->user->ice }}</p>
                </div>
            </div>
                ...
                <table class="w-full border-collapse">
                    ...
                    <tbody>
                        ...
                        @foreach ($devis->services as $index => $service)
                            @php
                                $reduction = $service->price * ($devis->reduction / 100);
                                $ht = $service->price - $reduction;
                                $tva = $ht * ($devis->tva / 100);
                                $ttc = $ht + $tva;
                                $total_ht += $ht;
                                $total_tva += $tva;
                                $total_ttc += $ttc;
